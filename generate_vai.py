...
"""
generate_vai.py
Module de génération de tickets VAI 9 BOULES — Ticket Bingo (TUKEA)
Grille 3×3 PLEINE (9 numéros), colonnes 61-70 / 71-80 / 81-90.
...
def _gen_nums(rng):
    c0 = sorted(rng.sample(range(61, 70+1), 3))
    c1 = sorted(rng.sample(range(71, 80+1), 3))
    c2 = sorted(rng.sample(range(81, 90+1), 3))
...
def _draw_ticket(cv, serial, grille, accent, couleur=True):
    ...
    cv.drawCentredString(MARGIN + CARD_W / 2, grid_top + (HEAD_H - 9) / 2,
                         "Le jeux VAI pour 9 boules by TUKEA 89 22 23 05")
    for r in range(3):
        for c in range(3):
...
def generate_pdf(nb_tickets=500, serie_start=1, output_path="/data/VAI.pdf", couleur=True):
...
def generate_pdf_nb(nb_tickets=500, serie_start=1, output_path="/data/VAI_NB.pdf"):
    return generate_pdf(nb_tickets, serie_start, output_path, couleur=False)


if __name__ == "__main__":
    generate_pdf(nb_tickets=4, serie_start=1, output_path="vai_test.pdf")
    print("VAI 9 BOULES genere")
