# -*- coding: utf-8 -*-
"""generate_sun.py — Tickets SUN 8 boules — Ticket Bingo (TUKEA)
Grille 3×3, colonnes 1-8 / 9-16 / 17-24. Colonne du milieu : 1 case vide (aléatoire).
8 numéros. 1 page = 1 ticket. Chiffres NOIRS, cadre COULEUR arc-en-ciel."""
import random
...
def _gen_nums(rng):
    ...
    c1=[]; it=iter(c1n)
    for r in range(3): c1.append(None if r==vide else next(it))
...
def _draw_ticket(cv, serial, grille, accent, couleur=True):
    ...
    cv.drawCentredString(MARGIN+CARD_W/2,grid_top+(HEAD_H-9)/2,"Le jeux SUN pour 8 boules by TUKEA 89 22 23 05")
    for r in range(3):
        for c in range(3):
            v=grille[r][c]
...
def generate_pdf(nb_tickets=500, serie_start=1, output_path="/data/SUN.pdf", couleur=True):
...
def generate_pdf_nb(nb_tickets=500, serie_start=1, output_path="/data/SUN_NB.pdf"):
    return generate_pdf(nb_tickets,serie_start,output_path,couleur=False)

if __name__=="__main__":
    generate_pdf(nb_tickets=4,serie_start=1,output_path="sun_test.pdf"); print("SUN genere")
