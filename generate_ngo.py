# -*- coding: utf-8 -*-
"""generate_ngo.py — Tickets NGO 8 BOULES — Ticket Bingo (TUKEA)
Grille 3×3, colonnes 31-45 / 46-60 / 61-75. CENTRE vide. 8 numéros.
...
def _gen_nums(rng):
    c0=sorted(rng.sample(range(31,45+1),3))
    c2=sorted(rng.sample(range(61,75+1),3))
    c1n=sorted(rng.sample(range(46,60+1),2))
    c1=[c1n[0],None,c1n[1]]
...
def _draw_ticket(cv, serial, grille, accent, couleur=True):
    ...
    cv.drawCentredString(MARGIN+CARD_W/2,grid_top+(HEAD_H-9)/2,"Le jeux NGO pour 8 boules by TUKEA 89 22 23 05")
    for r in range(3):
        for c in range(3):
            v=grille[r][c]
...
def generate_pdf(nb_tickets=500, serie_start=1, output_path="/data/NGO 8 BOULES.pdf", couleur=True):
...
def generate_pdf_nb(nb_tickets=500, serie_start=1, output_path="/data/NGO 8 BOULES_NB.pdf"):
    return generate_pdf(nb_tickets,serie_start,output_path,couleur=False)

if __name__=="__main__":
    generate_pdf(nb_tickets=4,serie_start=1,output_path="ngo_test.pdf"); print("NGO 8 BOULES genere")
