...
"""
generate_win.py
Module de génération de tickets WIN 9 BOULES — Ticket Bingo (TUKEA)
Grille 3×3 PLEINE (9 numéros), colonnes 1-15 / 16-30 / 31-45.
...
def _gen_nums(rng):
    ...
    c1 = sorted(rng.sample(range(16, 30+1), 3))
    c2 = sorted(rng.sample(range(31, 45+1), 3))
...
def _draw_ticket(cv, serial, grille, accent, couleur=True):
    ...
    cv.drawCentredString(MARGIN + CARD_W / 2, grid_top + (HEAD_H - 9) / 2,
                         "Le jeux WIN pour 9 boules by TUKEA 89 22 23 05")
    for r in range(3):
        for c in range(3):
...
def generate_pdf(nb_tickets=500, serie_start=1, output_path="/data/WIN.pdf", couleur=True):
...
def generate_pdf_nb(nb_tickets=500, serie_start=1, output_path="/data/WIN_NB.pdf"):
    return generate_pdf(nb_tickets, serie_start, output_path, couleur=False)


if __name__ == "__main__":
    generate_pdf(nb_tickets=4, serie_start=1, output_path="win_test.pdf")
    print("WIN 9 BOULES genere")
